...
@section('content')
        ...
        @endif
        <form method="POST" action="{{ isset($menu) ? route('menus.create', $menu->id) : route('menus.store') }}">
            @csrf
            @if (isset($menu))
                @method('PUT')
            @endif
            ...
            <div class="form-group" id="url_input">
                ...
                <input type="url" name="link" class="form-control" value="{{ old('link', isset($menu) ? $menu->link : '') }}">
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            toggleLinkFields(); // Initial call to set the correct display
        });
    </script>
    </div>
